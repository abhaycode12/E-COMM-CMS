...
const Dashboard: React.FC<DashboardProps> = ({ notify }) => {
  ...
  const handleExport = () => {
    notify?.("Preparing system-wide financial report...", "loading");
    setTimeout(() => notify?.("Global overview exported to Secure Drive.", "success"), 2000);
  };

  const handlePreparePO = () => {
    notify?.("Analyzing inventory burn rates for replenishment...", "loading");
    setTimeout(() => notify?.("Draft Purchase Order #PO-442 generated. Verify in Inventory module.", "success"), 1500);
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <button onClick={handleExport} className="px-6 py-3 border border-gray-200 rounded-2xl bg-white text-gray-600 font-bold hover:bg-gray-50 transition-all active:scale-95">Export Report</button>
          <button onClick={() => notify?.("Redirecting to Node Initialization...", "info")} className="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all active:scale-95">Add Product</button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
          <div className="flex justify-between items-center mb-8">
            ...
            <select className="text-xs font-black uppercase tracking-widest bg-gray-50 border-none rounded-xl px-4 py-2 outline-none">
        ...
        <div className="bg-indigo-900 text-white p-10 rounded-[2.5rem] shadow-2xl shadow-indigo-100 overflow-hidden relative flex flex-col justify-between">
          ...
          <div className="mt-12 p-6 bg-white/5 rounded-3xl border border-white/10 relative z-10">
             ...
             <button onClick={handlePreparePO} className="mt-4 text-[10px] font-black bg-white text-indigo-900 px-4 py-2 rounded-xl hover:bg-indigo-50 transition-all active:scale-95">Yes, Prepare PO</button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
          ...
          <div className="space-y-4">
            {[1, 2, 3, 4].map(i => (
              <div key={i} onClick={() => notify?.(`Opening manifest audit for Order #32${i}1...`, "info")} className="flex items-center justify-between p-5 border border-gray-50 rounded-[2rem] hover:bg-gray-50 cursor-pointer transition-all group active:scale-[0.98]">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase mt-1">Sarah Jenkins • {i * 2} mins ago</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-black text-gray-900">${(249 * i / 2).toFixed(2)}</p>
