    ...
    <div className="space-y-10">
      ...
      {selectedIds.length > 0 && (
        <div className="sticky top-6 z-[60] bg-gray-900 text-white p-5 rounded-[2rem] shadow-2xl flex items-center justify-between animate-in slide-in-from-top-6 duration-500">
          ...
          <button onClick={() => setSelectedIds([])} className="text-gray-400 hover:text-white px-4">Clear Selection</button>
        </div>
      )}
          ...
          <table className="w-full text-left min-w-[1000px]">
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredOrders.map(order => (
                <tr key={order.id} className={`hover:bg-indigo-50/10 transition-all duration-300 group cursor-pointer ${selectedIds.includes(order.id) ? 'bg-indigo-50/30' : ''}`}>
